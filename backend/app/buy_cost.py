...
PRESET_AUCTION_HOUSES = [
    {
        "name": "🏛️ Encore Auctions (London ON)",
        ...
        "currency": "CAD",
        "notes": "Local London auction house with online bidding. Use for weekly auctions with general merchandise, returns, and liquidation lots.",
        "is_default": 1,
    },
    {
        ...
        "online_bidding_fee_pct": 0.0,
        "payment_methods": "etransfer,cash",
        "currency": "CAD",
        "notes": "Buying directly from wholesalers or liquidation companies in bulk. No buyer premium, just 13% HST on the hammer price.",
        "is_default": 0,
    },
    {
        "name": "📦 Estate Sale / Liquidation",
        ...
        "currency": "CAD",
        "notes": "Estate sales and liquidation auctions. Typical 10% premium plus HST. Great for vintage items, collectibles, and household goods.",
        "is_default": 0,
    },
    {
        "name": "🤝 Private Seller / Kijiji",
        "buyer_premium_pct": 0.0,
        "handling_fee_flat": 0.0,
        "handling_fee_pct": 0.0,
        "handling_fee_mode": "none",
        ...
        "currency": "CAD",
        "notes": "Cash deals with private sellers (no HST charged). Use for Kijiji, Craigslist, garage sales, and peer-to-peer deals.",
        "is_default": 0,
    },
    {
        ...
        "payment_methods": "cash,etransfer",
        "currency": "CAD",
        "notes": "Local pickup from Facebook Marketplace. No fees, no tax. Factor in your time and gas for pickup.",
        "is_default": 0,
    },
    {
        "name": "🤝 Private Seller / Kijiji",
    ...
    {
        "name": "⚙️ Custom/Reseller Buy Cost",
        "buyer_premium_pct": 0.0,
        "handling_fee_flat": 0.0,
        "handling_fee_pct": 0.0,
        "handling_fee_mode": "none",
        ...
        "credit_card_surcharge_pct": 0.0,
        "online_bidding_fee_pct": 0.0,
        "payment_methods": "cash,etransfer,credit_card",
        "currency": "CAD",
        "notes": "Your personal editable template. Customize with your own fee structure and save as your default.",
        "is_default": 0,
    },
]
...
def build_config_with_settings(settings: dict, base_config: Optional[dict] = None) -> dict:
    """
    Build a buy-cost config merging user settings (custom tax override).
    If use_custom_tax=1, overrides tax_pct with custom_tax_pct.
    """
    config = dict(base_config or get_preset_config("Encore Auctions (London ON)"))
    use_custom = settings.get("use_custom_tax")
    if use_custom and str(use_custom) == "1":
        try:
            config["tax_pct"] = float(settings.get("custom_tax_pct", config["tax_pct"]))
        except (ValueError, TypeError):
            pass
    return config


def get_tax_label(settings: dict) -> str:
    """Return the display label for the tax line (HST, GST, or custom label)."""
    if str(settings.get("use_custom_tax", "0")) == "1":
        label = settings.get("custom_tax_label", "")
        return label if label else "Custom Tax"
    province = settings.get("province", "ON")
    info = CANADA_TAX_RATES.get(province, CANADA_TAX_RATES["ON"])
    return info["type"]
